...
ll solve(string &s, int index, int count3, int count6, int count9, int isPrefixEqual) {
	if (dp[index][count6 - count3][count9 - count6][isPrefixEqual] != -1)
		return dp[index][count6 - count3][count9 - count6][isPrefixEqual];
	ll res = 0;
	if (index == n) {
	...
	dp[index][count6 - count3][count9 - count6][isPrefixEqual] = res;
	return res;
}
